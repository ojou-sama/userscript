...
export const settingsUI = {
  create(modules = []) {
    ...
    const attachToDOM = () => { if (!document.body.contains(wrapper)) document.body.appendChild(wrapper); };
    ...
    const uiHelper = {
      ...
      buildNumber(label, val, min, max, onChange) {
        const div = document.createElement('div');
        div.className = 'account-edit-entry oa-input-wrapper';
        div.innerHTML = `<input class="account-edit-entry__input" type="number" value="${val || 0}" ${min !== undefined ? `min="${min}"` : ''} ${max !== undefined ? `max="${max}"` : ''}><div class="account-edit-entry__label">${label}</div>`;
      ...
      buildAsset(label, val, placeholder, onChange) {
        ...
        inputEl.addEventListener('input', (e) => {
          ...
          if (!url) { errorEl.style.display = 'none'; onChange(url); return; }
          
          const img = new Image();
          img.onload = () => { errorEl.style.display = 'none'; onChange(url); };
          img.onerror = () => { errorEl.style.display = 'block'; }; 
          img.src = url;
        });
        return div;
      }
    ...
    apiActions.style.display = 'flex'; apiActions.style.gap = '10px';
    ...
    apiActions.querySelector('#oa-btn-save').addEventListener('click', () => {
      ...
      alert('OAuth Credentials Saved. Page will reload.'); location.reload();
    });
    apiActions.querySelector('#oa-btn-clear').addEventListener('click', () => {
      settings.set('osu_client_id', ''); settings.set('osu_client_secret', '');
      alert('Credentials Cleared.'); location.reload();
    });
